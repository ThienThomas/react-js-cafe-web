...
const OrderHistory = () => {
  ...
  return !order || order?.length === 0 ? (
    <div className=" h-1/2">
      <img src={require('../../assets/images/order-go-here.webp')} className="mx-auto" />
      <p className="text-center">Đơn hàng đã đặt của bạn sẽ xuất hiện ở đây !</p>
    </div>
  ) : (
    <div>
      <div className="">
              ...
              {order.map((product: any) => {
                ...
                return (
                  <tr
                    ...
                    <td className="py-4 px-6 font-semibold text-gray-900 dark:text-white">
                      {product.totalPrice} đ
                    </td>
                    <td className="py-4 px-6">
                      <div className="font-medium text-sm">
                        {product.status === 'PROCESSED' ? (
                          <span className="p-2 bg-green-500/60 rounded-2xl text-white">
                            {product.status}{' '}
                          </span>
                        ) : (
                          <span className="p-2 bg-orange-500 rounded-2xl text-white">
                            {product.status}
                          </span>
                        )}
                      </div>
